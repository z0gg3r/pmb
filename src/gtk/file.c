...
selective_dialog_content(GtkWidget* window, char* action)
{
  ...
  g_signal_connect
    (GTK_WIDGET(select_button), "clicked", G_CALLBACK(selective_get_database)
     ,window);
  ...
  gtk_tree_model_get_iter_first
    (GTK_TREE_MODEL((gtk_combo_box_get_model(GTK_COMBO_BOX(field_box))))
     ,&iter);
  ...
  g_signal_connect
    (GTK_WIDGET(apply_button), "clicked", G_CALLBACK(selective_copy)
     ,action);
