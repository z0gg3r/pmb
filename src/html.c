...
char *bookmark_row(bookmark *b)
{
	...
	if (comment) {
		size = strlen(link) + strlen(bookmark_url(b)) +
		       strlen(bookmark_name(b)) + strlen(favicon_img) +
		       strlen(comment) + 1;
		...
		size = strlen(link) + strlen(bookmark_url(b)) +
		       strlen(bookmark_name(b)) + strlen(favicon_img) + 1;
...
int html_tree_table_row(directory *d, int depth, FILE *fp)
{
	if (d) {
		bookmark *b = NULL;
		char *open_ul = "<ul>\n";
...
void html_tree_branch(directory *d, int depth, FILE *fp)
{
	if (d) {
		directory_rewind(d);
		...
		while ((ret = directory_next_children(d))) {
			...
			html_tree_branch(directory_next_children(ret),
					 depth + 1, fp);
		}
...
int bookmark_html_tree(bookmark_list *bl, FILE *fp)
{
	if (bl) {
