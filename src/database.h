...
sqlite3 *bookmark_db_open(char *);

/* sqlite3 database */
void bookmark_db_close(sqlite3 *);

/* bookmark* bookmark, sqlite3* database */
int bookmark_db_write(bookmark *, sqlite3 *);

/* -- import one database into another (second into the first) */
/* sqlite3* database, sqlite3* database */
int bookmark_db_import(sqlite3 *, sqlite3 *);

/* sqlite3* database, int item id */
int bookmark_db_delete(sqlite3 *, int);
