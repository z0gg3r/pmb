...
void bookmark_print_field(unsigned int i, char *r)
{
...
void bookmark_print(bookmark_list *bl, int field)
{
	bookmark *b = NULL;
	unsigned int i = 0;
			...
			if (verbose) {
				if (color && isatty(STDOUT_FILENO)) {
					printf("id\t\t%s%s%s\n"
					       ...
					       name_color, bookmark_name(b),
					       reset, url_color,
					       bookmark_url(b), reset,
					       comment_color,
					       bookmark_comment(b), reset,
					       tag_color, bookmark_tag(b),
					       reset);
				} else {
					printf("id\t\t%s\n"
					       "name\t\t%s\n"
...
{
	if ((!strcmp(color, "red") || (!strcmp(color, "Red")))) {
		return red;
	}

	else if ((!strcmp(color, "blue") || (!strcmp(color, "Blue")))) {
		return blue;
	}

	else if ((!strcmp(color, "cyan") || (!strcmp(color, "Cyan")))) {
		return cyan;
	}

	else if ((!strcmp(color, "green") || (!strcmp(color, "Green")))) {
		return green;
	}

	else if ((!strcmp(color, "reset") || (!strcmp(color, "Reset")))) {
		return reset;
	}

	else if ((!strcmp(color, "white") || (!strcmp(color, "White")))) {
		return white;
	}

	else if ((!strcmp(color, "magenta") || (!strcmp(color, "Magenta")))) {
		return magenta;
	}

	else if ((!strcmp(color, "gray") || (!strcmp(color, "Gray")))) {
		return gray;
	}

	else if ((!strcmp(color, "yellow") || (!strcmp(color, "Yellow")))) {
		return yellow;
	}
...
int parse_config_file(char *optarg)
{
	char **res = NULL;
...
int add_bookmark(char *optarg)
{
	if (g_db) {
		...
		char *const sub_options[] = {
			[name_option] = NAME,	      [sname_option] = "n",
...
int edit_bookmark(char *optarg)
{
	if (g_db) {
		...
		char *const sub_options[] = { [id_option] = ID,
					      [sid_option] = "i",
					      [name_option] = NAME,
					      [sname_option] = "n",
					      [url_option] = URL,
		...
		if (field && !strncmp(field, "comment", 7) && c_value &&
		    n_value) {
...
int search(char *optarg)
{
	if (g_db) {
		...
		char *const sub_options[] = { [name_option] = NAME,
					      [sname_option] = "n",
					      [url_option] = URL,
					      [surl_option] = "u",
					      [comment_option] = COMMENT,
		...
		while (*subopts != '\0')
			...
			case sname_option:
				if (value) {
					bl = bookmark_db_search(g_db, NAME,
								value);
				} else {
			...
			case surl_option:
				if (value) {
					bl = bookmark_db_search(g_db, URL,
								value);
				} else
			...
			case stag_option:
				if (value) {
					bl = bookmark_db_search(g_db, TAG,
								value);
				} else {
			...
			default:
				if (value) {
					bl = bookmark_db_search(g_db, NULL,
								value);
				} else {
		...
		if (bl) {
			...
			if (export) {
				...
				if (e_db) {
					...
					while ((b = bookmark_list_return_next_bookmark(
							bl))) {
						if (bookmark_db_write(b,
								      e_db)) {
			...
			if (del) {
				...
				while ((b = bookmark_list_return_next_bookmark(
						bl))) {
					int id = strtol(bookmark_id(b), NULL,
							10);
...
int print_bookmark(char *optarg)
{
	if (g_db) {
		int id = 0;
		char *field = NULL;
		...
		if (field) {
			...
			if (!strncmp(field, COMMENT, strlen(COMMENT)) ||
			    (!strncmp(field, "c", 1))) {
				if (id) {
					bl = bookmark_db_query(g_db, id,
							       COMMENT);
				} else {
					bl = bookmark_db_query(g_db, 0,
							       COMMENT);
				}
...
int delete_bookmark(char *optarg)
{
	if (g_db) {
		int id = 0;
		int greedy = 0;
...
int set_color(char *optarg)
{
	color ? (color = 0) : (color = 1);
	return 0;
}

int set_verbose(char *optarg)
{
	verbose ? (verbose = 0) : (verbose = 1);
	return 0;
}

void parse_options(int argc, char *argv[], cl_option_list *option,
		   cl_option_list *command)
{
	...
	static struct option long_options[] = {
		{ "add", 1, 0, 'a' },	    { "color", 0, 0, 'c' },
		{ "colors", 1, 0, 'l' },    { "config", 1, 0, 'C' },
		{ "delete", 1, 0, 'd' },    { "edit", 1, 0, 'e' },
		{ "open-file", 1, 0, 'f' }, { "import", 1, 0, 'i' },
		{ "help", 0, 0, 'h' },	    { "html", 0, 0, 'H' },
		{ "print", 1, 0, 'p' },	    { "search", 1, 0, 's' },
		{ "verbose", 0, 0, 'V' },   { "version", 0, 0, 'V' },
		{ NULL, 0, NULL, 0 }
	};

	while ((c = getopt_long(argc, argv, "a:C:e:d:s:f:i:p:l:chHVv",
		...
		case 'a': /* add */
		{
			cl_option *opt = option_new(add_bookmark, optarg);
		...
		case 'C': /* config */
		{
			cl_option *opt = option_new(parse_config_file, optarg);
		...
		case 'c': /* color */
		{
			cl_option *opt = option_new(set_color, optarg);
			option_list_add(option, opt);
		...
		case 'd': /* delete */
		{
			cl_option *opt = option_new(delete_bookmark, optarg);
			option_list_add(command, opt);
		...
		case 'e': /* edit */
		{
			cl_option *opt = option_new(edit_bookmark, optarg);
			option_list_add(command, opt);
		...
		case 'f': /* open file */
		{
			cl_option *opt = option_new(open_file, optarg);
			option_list_add(option, opt);
		...
		case 'i': /* import file */
		{
			cl_option *opt = option_new(import, optarg);
			option_list_add(command, opt);
		...
		case 'l': /* color */
		{
			cl_option *opt = option_new(color_list, optarg);
			option_list_add(option, opt);
		...
		case 'p': /* print */
		{
			cl_option *opt = option_new(print_bookmark, optarg);
			option_list_add(command, opt);
		...
		case 'v': /* verbose */
		{
			cl_option *opt = option_new(set_verbose, optarg);
			option_list_add(option, opt);
